...
user_setup:
  - service: supabase
    ...
    dashboard_config:
      - task: "Utwórz nowy projekt Supabase"
...
must_haves:
  truths:
    - "Klient Supabase działa w przeglądarce"
    - "Klient Supabase działa na serwerze (Server Components)"
    - "Middleware odwieza sesje przy każdym uzyciu"
    - "Dark mode przelacza się poprawnie i persystuje po odswiezeniu"
    - "shadcn/ui komponenty są dostępne do uzycia"
  artifacts:
...
Purpose: Fundament dla auth flows (Plan 02) i protected routes (Plan 03). Bez tego ani login, ani dashboard nie będą działać.
...
Utwórz klienty Supabase zgodnie z wzorcami z RESEARCH.md:

1. **src/lib/supabase/client.ts** - Browser client:
   - Eksportuj funkcje `createClient()` uzywajaca `createBrowserClient`
   - Użyj env vars: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`
...
3. **src/lib/supabase/middleware.ts** - Middleware helper:
   ...
   - Użyj `createServerClient` z cookie handlers dla request/response
   - Wywolaj `supabase.auth.getUser()` (NIE getSession - nie rewaliduje tokenów!)
   - Zwroc zmodyfikowany response

4. **src/lib/dal/auth.ts** - Data Access Layer:
   - `getUser()` - cached per-request (użyj React `cache()`), zwraca User | null
   - `requireAuth()` - wywoluje getUser, redirect do /login jeśli null
   - `verifySession()` - wywoluje getUser, throw Error jeśli null (dla Server Actions)
...
WAŻNE:
- NIE blokuj dostępu w middleware (CVE-2025-29927)
...
npm run build
```
Build musi przejść bez błędów TypeScript. Sprawdź czy wszystkie eksporty są poprawne.
  </verify>
  <done>
...
5. **src/components/layout/theme-toggle.tsx**:
   ...
   - Użyj ikony Sun/Moon z lucide-react
   - Użyj `useTheme` z next-themes

6. **src/app/globals.css** - Dark mode dla Tailwind v4:
   - Dodaj na początku: `@custom-variant dark (&:where(.dark, .dark *));`
...
WAŻNE dla Tailwind v4:
- NIE używaj tailwind.config.js dla darkMode
- Użyj @custom-variant w CSS
  </action>
  <verify>
```bash
npm run dev
...
3. Theme powinien się zmienic (tlo strony)
4. Odśwież strone - theme powinien być zachowany
  </verify>
  <done>
- shadcn/ui zainicjalizowane z komponentami: button, card, input, label, form, avatar
- ThemeProvider dodany do root layout
- ThemeToggle działa i persystuje wybor
...
5. Pliki Supabase eksportuja poprawne funkcje (sprawdź importy)
</verification>

<success_criteria>
- Klienty Supabase gotowe do uzycia w auth flows (Plan 02)
- DAL z requireAuth gotowy do ochrony stron (Plan 03)
- shadcn/ui komponenty dostępne dla formularzy auth
- Dark mode działa i persystuje
- Middleware odswieza sesje automatycznie
</success_criteria>

<output>
Po zakończeniu utwórz `.planning/phases/01-auth-basic-ui/01-01-SUMMARY.md`
</output>
