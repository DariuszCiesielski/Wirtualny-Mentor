...
must_haves:
  truths:
    - "Użytkownik może zapisac spersonalizowany kurs do bazy danych"
    - "Postep użytkownika jest przechowywany między sesjami"
    - "Użytkownik widzi tylko swoje kursy (inni nie mają dostępu)"
    - "Kurs zawiera zagniezdzone poziomy, outcomes i rozdziały"
  artifacts:
...
<objective>
Schemat bazy danych dla kursów, poziomow, rozdziałów i postępu użytkownika.

Purpose: Fundament persystencji dla całej fazy 2 - bez bazy danych nie można zapisywac curriculum ani sledzic postępu.
...
<task type="auto">
  ...
  <action>
Utwórz migracje SQL zgodnie z patternem z RESEARCH.md:
...
1. Tabela `courses`:
   ...
   - prerequisites TEXT[]
   - source_url TEXT (opcjonalny link źródłowy)
   - user_goals TEXT[]
...
2. Tabela `course_levels` (5 per course):
   ...
   - name TEXT CHECK (name IN ('Początkujący', 'Średnio zaawansowany', 'Zaawansowany', 'Master', 'Guru'))
   - description TEXT
...
7. RLS Policies:
   ...
   - Similar policies dla related tables (poziomy, rozdziały - przez course_id join)
...
W `src/types/database.ts` utwórz TypeScript types matching schema (Course, CourseLevel, LevelOutcome, Chapter, UserProgress, CourseResource).
  </action>
  <verify>
Sprawdź że plik migracji jest poprawnym SQL:
...
- `npx tsc --noEmit` - brak błędów TypeScript w types/database.ts
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
Utwórz Data Access Layer dla curriculum:
...
2. `saveCurriculumWithLevels(courseId: string, curriculum: CurriculumData): Promise<void>`
   ...
   - Utwórz initial user_progress record (current_level = pierwszy, current_chapter = pierwszy)

3. `getCourse(courseId: string): Promise<CourseWithDetails | null>`
   - Select course z nested levels, outcomes, chapters
   - Użyj Supabase select z relacjami
...
1. `getProgress(userId: string, courseId: string): Promise<UserProgress | null>`
   - Pobierz postęp użytkownika dla kursu
...
3. `markChapterComplete(progressId: string, chapterId: string): Promise<void>`
   ...
   - Sprawdź czy wszystkie rozdziały poziomu ukończone -> dodaj level do completed_levels

4. `calculateProgressPercentage(progress: UserProgress, course: CourseWithDetails): number`
   - Oblicz % ukończenia (completed_chapters / total_chapters)

Wszystkie funkcje uzywaja `createClient()` z `@/lib/supabase/server` i są async.
Wszystkie funkcje mają TypeScript types dla inputs/outputs.
  </action>
  <verify>
- `npx tsc --noEmit` - brak błędów TypeScript
- Sprawdź eksporty: `grep -c "export async function" src/lib/dal/courses.ts` >= 4
- Sprawdź eksporty: `grep -c "export async function" src/lib/dal/progress.ts` >= 3
  </verify>
  <done>
