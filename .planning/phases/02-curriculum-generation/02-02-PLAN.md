...
must_haves:
  truths:
    - "Tavily client może wykonac web search i zwrocic wyniki"
    - "Zod schemas waliduja strukture curriculum z 5 poziomami"
    - "System prompts są gotowe dla clarifying questions i curriculum generation"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/lib/ai/curriculum/schemas.ts"
      ...
      pattern: "levelSchema.*name.*Początkujący"
...
user_setup:
  - service: tavily
    ...
    account_setup:
      - task: "Utwórz konto na tavily.com"
...
Purpose: Przygotowanie narzędzi AI - search client dla aktualnych informacji i schemas do walidacji structured output.
...
2. Utwórz `src/lib/tavily/client.ts`:
   ```typescript
...
3. Utwórz `src/app/api/search/route.ts`:
   ```typescript
...
4. Dodaj do `.env.example`:
  ...
  <verify>
- `npm run build` - brak błędów kompilacji
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/lib/ai/curriculum/schemas.ts`:
   ```typescript
   import { z } from 'zod';
...
2. Utwórz `src/lib/ai/curriculum/prompts.ts`:
   ```typescript
...
- `grep "length(5)" src/lib/ai/curriculum/schemas.ts` - schema wymaga 5 poziomow
  ...
  <done>
Zod schemas: userInfoSchema, clarificationSchema, curriculumSchema (z dokładnie 5 poziomami).
...
- [ ] API endpoint /api/search działa z walidacja Zod
- [ ] curriculumSchema wymaga dokładnie 5 poziomow
- [ ] System prompts są po polsku
