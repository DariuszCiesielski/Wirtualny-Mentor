...
must_haves:
  truths:
    - "Użytkownik może wpisac temat nauki lub podac link do źródła"
    - "AI zadaje pytania doprecyzowujące i zbiera informacje o uzytkowniku"
...
Purpose: Użytkownik może rozpoczac tworzenie kursu od wpisania tematu lub linku. AI zadaje pytania doprecyzowujące aby zebrać informacje potrzebne do generowania curriculum.
Output: Strona /courses/new z topic input i chat interface dla pytań AI.
</objective>
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/components/curriculum/topic-input.tsx`:
   - Client component ('use client')
   - Formularz z dwoma polami:
     a) Textarea dla tematu nauki (placeholder: "Czego chcesz się nauczyć? np. 'programowanie w Python' lub 'prawo cywilne w Polsce'")
     b) Input dla opcjonalnego URL źródła (placeholder: "Opcjonalnie: link do materialu, na podstawie którego chcesz się uczyć")
   - Walidacja: temat min 3 znaki, URL opcjonalny ale jeśli podany to valid URL
   - onSubmit callback z (topic: string, sourceUrl?: string)
   - Przycisk "Rozpocznij" disabled gdy temat < 3 znaki
   - Użyj shadcn/ui: Button, Input, Card, Textarea (dodaj jeśli brak)

2. Utwórz `src/app/(dashboard)/courses/new/page.tsx`:
...
3. Utwórz `src/app/(dashboard)/courses/new/actions.ts`:
   - Server action `initiateCourseCreation` - tworzy draft course w bazie
   - Input: userId, topic, sourceUrl
   - Output: courseId
   - Status kursu: 'draft'

4. Jeśli brak komponentu Textarea w shadcn, dodaj go:
   ```bash
...
- Strona /courses/new renderuje się bez błędów
  </verify>
  <done>
TopicInput component z walidacja tematu i opcjonalnego URL.
Strona /courses/new z state machine dla 4 kroków.
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/app/api/curriculum/clarify/route.ts`:
   ```typescript
...
2. Utwórz `src/components/curriculum/clarifying-chat.tsx`:
   - Client component ('use client')
   - Props: topic, sourceUrl?, onComplete(userInfo)
   - Użyj `useChat` z '@ai-sdk/react':
     ```typescript
   ...
   - Wyswietl historię wiadomości:
     ...
     - Jeśli AI podaje options, wyswietl jako klikalne buttony
...
3. Polacz z page.tsx:
  ...
  <verify>
- `npm run build` - brak błędów
- `grep "useChat" src/components/curriculum/clarifying-chat.tsx` - używa hooka
- `grep "streamText" src/app/api/curriculum/clarify/route.ts` - używa streaming
...
- [ ] ClarifyingChat wyświetla historię wiadomości i options
- [ ] Po isComplete: true przechodzi do następnego kroku
