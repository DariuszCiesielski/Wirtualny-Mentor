...
must_haves:
  truths:
    - "AI generuje curriculum z dokładnie 5 poziomami (Początkujący -> Guru)"
    - "Każdy poziom ma learning outcomes i rozdziały"
    - "Użytkownik widzi streaming progress podczas generowania"
    - "Wygenerowane curriculum można zapisac do bazy danych"
...
Purpose: Core funkcjonalność fazy - AI tworzy spersonalizowany program nauczania na podstawie zebranych informacji. Użytkownik widzi streaming progress i może zapisac wynik.
...
<task type="auto">
  ...
  <action>
Utwórz `src/app/api/curriculum/generate/route.ts`:
...
- extractUrls dla source URL użytkownika
- maxTokens: 8192 dla duzego curriculum
- Streaming response z Output.object
  </action>
  <verify>
- `npm run build` - brak błędów
- `grep "streamText" src/app/api/curriculum/generate/route.ts` - używa streaming
- `grep "searchWeb" src/app/api/curriculum/generate/route.ts` - integruje Tavily
- `grep "maxTokens" src/app/api/curriculum/generate/route.ts` - ma limit tokenów
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/components/curriculum/curriculum-generator.tsx`:
   - Client component
   - Props: userInfo, courseId, onComplete(curriculum)
   - Użyj `useObject` z '@ai-sdk/react' lub custom fetch z streaming:
     ```typescript
     'use client';
     ...
     ```
   - Wyswietl postęp generowania:
     - Tytul kursu (gdy dostępny)
     - Lista poziomow (pojawiaja się stopniowo)
     - Dla każdego poziomu: nazwa, opis, liczba rozdziałów
   - Loading spinner i komunikaty o postepie
   - Error handling z przyciskiem "Sprobuj ponownie"

2. Utwórz `src/components/curriculum/curriculum-preview.tsx`:
   - Client component
   - Props: curriculum, onConfirm, onEdit (optional)
   - Wyswietl pełna strukture curriculum:
     - Tytul i opis kursu
     - Szacowany całkowity czas
     - Prerequisites lista
     - Accordion/Collapsible dla każdego z 5 poziomow:
       - Nazwa poziomu i opis
       - Lista learning outcomes z checkmarks
       - Lista rozdziałów z czasem
...
4. Polacz wszystko w page.tsx:
   ...
   - Po onConfirm: wywołaj saveCurriculum i redirect
  </action>
  <verify>
- `npm run build` - brak błędów
...
CurriculumPreview: accordion z 5 poziomami, learning outcomes, rozdziałami.
...
- [ ] Generator wyświetla postęp streaming
- [ ] Preview pokazuje wszystkie 5 poziomow z learning outcomes
- [ ] Zapis do bazy działa (saveCurriculumWithLevels)
