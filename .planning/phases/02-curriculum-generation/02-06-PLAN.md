...
must_haves:
  truths:
    - "Postep użytkownika jest zapisywany po ukończeniu rozdziału"
    - "Użytkownik widzi pasek postępu (ile przeszedl z kursu)"
    - "Nawigacja między rozdziałami działa (następny/poprzedni)"
    - "Postep persystuje między sesjami"
  artifacts:
...
<objective>
Progress tracking i nawigacja między rozdziałami.

Purpose: Użytkownik może przegladac rozdziały, oznaczac je jako ukończone i nawigowac między nimi. Pasek postępu pokazuje całkowity progress w kursie.
Output: Strona rozdziału z placeholder na content, progress bar, przycisk "Ukoncz" i nawigacja.
</objective>
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/app/(dashboard)/courses/[courseId]/[levelId]/[chapterId]/actions.ts`:
   ```typescript
   'use server';
...
2. Utwórz `src/app/(dashboard)/courses/[courseId]/[levelId]/[chapterId]/page.tsx`:
   ```typescript
   import { requireAuth } from '@/lib/dal/auth';
   import { getCourse } from '@/lib/dal/courses';
...
<task type="auto">
  ...
  <action>
1. Utwórz `src/components/curriculum/progress-bar.tsx`:
   ```typescript
   'use client';
...
2. Utwórz `src/components/curriculum/chapter-navigation.tsx`:
   ```typescript
   'use client';
...
3. Dodaj do sidebar "Moje kursy" link (odblokuj):
  ...
  <verify>
- `npm run build` - brak błędów
- `grep "percentage" src/components/curriculum/progress-bar.tsx` - wyświetla procent
...
2. Nawiguj do /courses/[courseId]/[levelId]/[chapterId]:
   ...
   - Widoczny tytul i opis rozdziału
   - Przycisk "Ukoncz rozdział" zapisuje postęp
   - Po kliknieciu redirect do następnego rozdziału
   - Nawigacja prev/next działa
3. Odśwież strone - postęp persystuje (z bazy)
</verification>

<success_criteria>
- [ ] Chapter page wyświetla progress bar z procentem
- [ ] Przycisk "Ukoncz rozdział" zapisuje do bazy i redirect
- [ ] Nawigacja prev/next poprawnie oblicza kolejne rozdziały
- [ ] Po ostatnim rozdziale w kursie pokazuje "Zakoncz kurs"
- [ ] Progress persystuje między sesjami (odświeżenie strony)
