...
must_haves:
  truths:
    ...
    - "TypeScript types dla SectionContent i Source są dostępne"
    - "Zod schemas dla walidacji contentu są zdefiniowane"
  artifacts:
...
<objective>
Stwórz fundament bazy danych i typow dla systemu materiałów edukacyjnych.

Purpose: Przygotowanie infrastruktury storage i type safety dla generowanych materiałów. Tabela section_content przechowuje markdown content z metadanymi o źródłach. Typy TypeScript i Zod schemas zapewniaja spójność danych między AI generation a storage.
...
<task type="auto">
  ...
  <action>
Stwórz migration dla tabeli section_content:
...
WAŻNE:
- RLS policy używa tego samego pattern via course_id join co chapters
- JSONB dla elastycznosci (key_concepts, practical_steps, tools, external_resources, sources)
- Versioning umożliwia regeneracje bez utraty starej wersji
- GIN index na sources dla szybkiego wyszukiwania
  </action>
  <verify>
    - Plik migration istnieje: `ls supabase/migrations/20260131000001_section_content.sql`
    - SQL syntax valid: `npx supabase db lint` lub jeśli niedostepny: sprawdź czy plik parsuje się bez błędów skladni (CREATE TABLE, policy names, etc.)
    - TypeScript types sync: `npx tsc --noEmit` po wykonaniu Task 2
  </verify>
...
<task type="auto">
  ...
  <action>
1. Stwórz `src/types/materials.ts`:
...
2. Stwórz `src/lib/ai/materials/schemas.ts`:
...
WAŻNE:
- Typy mają camelCase (frontend), database row ma snake_case
- Zod schemas mają .describe() dla AI generation guidance
- sourceSchema wymaga url().datetime() dla walidacji
- Schema i types są zsynchronizowane
  </action>
  <verify>
    - `npx tsc --noEmit` - brak błędów TypeScript
    - Importy działają: `import { SectionContent } from '@/types/materials'`
  </verify>
...
2. `npx tsc --noEmit` przechodzi bez błędów
3. Typy można importowac z @/types/materials
4. Schemas można importowac z @/lib/ai/materials/schemas
</verification>

<success_criteria>
- [ ] Migration SQL poprawny skladniowo (verified via file existence + tsc)
