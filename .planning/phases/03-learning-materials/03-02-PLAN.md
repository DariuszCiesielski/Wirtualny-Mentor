...
must_haves:
  truths:
    - "AI może wyszukiwać aktualne źródła przez web search"
    - "Materiały zawieraja cytaty z weryfikowalnych źródeł"
    - "Treści są zgodne z najnowsza dokumentacja (nie starsze niz 2 lata)"
    - "Praktyczne przykłady zawieraja interpretacje oczekiwanych wyników"
  artifacts:
...
<objective>
Stwórz AI tools i prompts dla generowania materiałów edukacyjnych z grounding.

Purpose: Narzędzia AI SDK (tool calling) pozwalaja modelowi na wyszukiwanie aktualnych informacji przez Tavily przed generowaniem treści. Prompty zapewniaja spójny format outputu w stylu podręcznika z cytatami.

Output: Web search tools, extraction tools, system prompts dla materiałów
</objective>
...
<task type="auto">
  ...
  <action>
Stwórz `src/lib/ai/materials/prompts.ts` z trzema promptami:
...
- OBOWIĄZKOWE interpretacje wyników dla praktycznych przykładów (MAT-06)
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
Stwórz `src/lib/ai/materials/tools.ts` z narzedzami do wyszukiwania:
...
WAŻNE:
- Uzywa istniejącego Tavily client z @/lib/tavily/client
- tool() z AI SDK v6 dla proper typing
- searchDepth: 'advanced' dla lepszych wyników
- Limity content (8000 chars) aby nie przekroczyc context window
- Error handling z success flag
- Unique IDs dla źródeł (timestamp + index)
  </action>
  <verify>
    - `npx tsc --noEmit` - brak błędów TypeScript
...
- [ ] MATERIAL_GENERATION_PROMPT wymaga interpretacji wyników dla przykładów praktycznych (MAT-06)
- [ ] searchResourcesTool używa searchWeb z Tavily
- [ ] extractContentTool używa extractUrls z Tavily
