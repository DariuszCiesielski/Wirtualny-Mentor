...
export async function POST(request: NextRequest) {
  try {
    ...
    // Phase 1: Research - gather sources with tool calling
...
Uzyj narzedzia searchResources dla kazdego tematu.`,
    ...
    const uniqueSources = collectedSources.reduce<CollectedSource[]>((acc, source) => {
...
- Weryfikacja dostpu usera do chapter przed generacja
