...
must_haves:
  truths:
    - "API endpoint generuje materiały dla chapter"
    - "DAL zapisuje i pobiera content z bazy danych"
    - "Lazy generation - content tworzony tylko gdy potrzebny"
    - "Wygenerowane materiały zawieraja sekcję z interpretacja wyników"
  artifacts:
...
<objective>
Stwórz DAL i API endpoint dla generowania materiałów edukacyjnych.

Purpose: API endpoint używa multi-step tool calling (research phase -> generation phase) do tworzenia grounded content. DAL zapewnia lazy generation - content tworzony tylko gdy użytkownik otwiera chapter, nie przy generacji curriculum.
...
<task type="auto">
  ...
  <action>
Stwórz `src/lib/dal/materials.ts`:
...
<task type="auto">
  ...
  <action>
Stwórz `src/app/api/materials/generate/route.ts`:

UWAGA: Ten endpoint jest złożony (~200 linii) ale logicznie spójny. Zawiera:
- Research phase (tool calling dla zbierania źródeł)
- Generation phase (structured output z zebranych źródeł)
- Save phase (zapis do DB)

Jeśli napotykasz problemy podczas implementacji, skup się na jednej fazie na raz.
...
- Weryfikacja dostępu usera do chapter przed generacja
- Deduplikacja źródeł po URL
- Limit 10 źródeł dla context window
...
- Error handling z descriptive messages
  ...
  <verify>
    ...
    - Importy z @/lib/ai/materials/* działają
  </verify>
...
2. DAL functions dostępne w src/lib/dal/materials.ts
3. API endpoint dostępny pod /api/materials/generate
4. API używa tool calling do research phase
</verification>
...
- [ ] Źródła są deduplikowane i limitowane
- [ ] Generated content zawiera sekcję "Jak interpretowac wyniki" lub "Oczekiwany wynik" (weryfikacja MAT-06)
