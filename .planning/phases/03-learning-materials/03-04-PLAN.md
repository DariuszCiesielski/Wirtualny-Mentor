...
must_haves:
  truths:
    ...
    - "Citations [1] są zamieniane na linki do źródeł"
    - "Lista źródeł jest wyświetlana na koncu"
    - "Narzędzia wyświetlają się z linkami i instrukcjami"
  artifacts:
...
<objective>
Stwórz komponenty UI do wyswietlania wygenerowanych materiałów edukacyjnych.

Purpose: ContentRenderer renderuje markdown z syntax highlighting i zamienia citations [n] na linki. SourceList wyświetla liste źródeł. ToolCard prezentuje rekomendowane narzędzia z linkami.

Output: React components dla materiałów edukacyjnych
</objective>
...
- rehype-raw: Obsluga raw HTML w markdown (ostronie używać)
- rehype-sanitize: Sanityzacja HTML dla bezpieczenstwa (XSS protection)

WAŻNE: Nie używaj highlight.js CSS - użyj Tailwind prose classes dla styling.
  </action>
  <verify>
...
<task type="auto">
  ...
  <action>
Stwórz `src/components/materials/content-renderer.tsx`:
...
- ExternalLink icon dla linków zewnętrznych
...
<task type="auto">
  ...
  <action>
1. Stwórz `src/components/materials/source-list.tsx`:
...
2. Stwórz `src/components/materials/tool-card.tsx`:
...
WAŻNE:
- SourceList numeruje źródła od [1]
- Badge z typem źródła (Dokumentacja, Artykuł, etc.)
- ToolCard pokazuje install command jeśli dostępny
- Badge dla free/paid narzędzi
...
3. Komponenty w src/components/materials/ istnieją
...
- [ ] SourceList wyświetla numerowana liste źródeł
- [ ] ToolCard wyświetla narzędzie z install command
