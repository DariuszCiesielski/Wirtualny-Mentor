...
must_haves:
  truths:
    - "Użytkownik może rozwiązywać quiz z pojedynczymi pytaniami"
    - "Użytkownik widzi feedback po wyslaniu odpowiedzi"
    - "Użytkownik widzi podsumowanie wyników po zakończeniu"
...
<objective>
Zbudować UI dla rozwiązywania quizów z feedbackiem i wynikami.

Purpose: UI pozwala użytkownikowi na interakcje z quizami i widzenie wyników.
Output: Komponenty quiz, stroną quiz w chapter routing.
</objective>
...
- Komponenty używają shadcn/ui components
  </verify>
  <done>
...
Główny container zarzadzajacy flow quizu:
...
<task type="auto">
  ...
  <action>
Strona quizu dostępna po ukończeniu rozdziału:
...
3. `npx tsc --noEmit` - kompiluje bez błędów
4. Visual check: quiz page renderuje się poprawnie (manual test)
</verification>

<success_criteria>
- 4 komponenty quiz w src/components/quiz/
- Quiz page route działa
