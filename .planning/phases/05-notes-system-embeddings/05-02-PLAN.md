...
must_haves:
  truths:
    ...
    - "Funkcje używają text-embedding-3-small z providers.ts"
  artifacts:
...
Purpose: DAL i Server Actions będą potrzebowaly generować embeddingi przy tworzeniu/edycji notatek. Ten plan dostarcza reuzywalne funkcje.
...
- Eksportuj EMBEDDING_MODEL_ID dla zapisywania w DB (śledzenie version drift)
- Loguj usage dla cost tracking
  </action>
  <verify>
1. `npx tsc --noEmit src/lib/ai/embeddings.ts` - brak błędów
2. Sprawdz czy importy są poprawne (embed, embedMany, cosineSimilarity z 'ai')
  </verify>
  <done>
...
4. Funkcje używają AI SDK (embed/embedMany z 'ai')
</verification>
