...
must_haves:
  truths:
    - "Użytkownik widzi pole do tworzenia notatki na stronie rozdziału"
    - "Użytkownik może zapisać notatkę klikając przycisk"
    - "Użytkownik widzi listę swoich notatek dla rozdziału"
    - "Użytkownik może edytować istniejącą notatkę"
    - "Użytkownik może usunąć notatkę"
  artifacts:
...
<objective>
Implementacja komponentów UI dla systemu notatek na stronie rozdziału.

Purpose: Użytkownik musi móc tworzyć, przeglądać, edytować i usuwać notatki podczas nauki. Ten plan dostarcza kompletny UI.

Output: Komponenty notes/* zintegrowane ze stroną rozdziału.
</objective>
...
- Obsługa error z Server Action
- Clear content po successful create (nie edit)
- Disabled state podczas pending
  </action>
  <verify>
`npx tsc --noEmit src/components/notes/note-editor.tsx` - brak błędów
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
Utworz komponent note-card.tsx do wyświetlania pojedynczej notatki:
...
WAŻNE:
- Toggle między view mode i edit mode
- Confirm dialog przed usunięciem
- Polish date formatting
- whitespace-pre-wrap dla zachowania formatowania
  </action>
  <verify>
`npx tsc --noEmit src/components/notes/note-card.tsx` - brak błędów
  </verify>
  <done>
Komponent note-card gotowy do wyświetlania i edycji pojedynczej notatki.
  </done>
</task>
...
2. Zaktualizuj chapter page.tsx żeby pobierać notatki i renderować NotesList:
...
WAŻNE:
- NotesList jest client component (używa useState)
- initialNotes są pobierane server-side i przekazywane jako prop
- Optimistic updates: dodawanie/usuwanie bez czekania na revalidation
  </action>
  <verify>
1. `npx tsc --noEmit src/components/notes/notes-list.tsx` - brak błędów
2. Strona chapter powinna zawierać import NotesList i renderować go
  </verify>
  <done>
System notatek zintegrowany ze stroną rozdziału - użytkownik może tworzyć, przeglądać, edytować i usuwać notatki.
  </done>
</task>
...
6. UI pokazuje editor + listę notatek
</verification>
