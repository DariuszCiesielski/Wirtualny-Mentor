...
must_haves:
  truths:
    ...
    - "Content zajmuje pełna szerokość na mobile"
    - "Wszystkie elementy nawigacji są dostępne na mobile"
...
Purpose: Spełnienie wymagania UX-02 - aplikacja musi dzialac na urzadzeniach mobilnych
...
- `npm run build` przechodzi bez błędów
  ...
  <done>Hook useMediaQuery działa i Sheet component zainstalowany</done>
</task>
...
2. Zaktualizuj src/app/(dashboard)/layout.tsx:
   - Zmien `pl-60` na `lg:pl-60` (pełna szerokość na mobile)
...
3. Opcjonalnie zaktualizuj src/components/layout/header.tsx:
  ...
  <verify>
- `npm run build` przechodzi bez błędów
- Na szerokości <1024px sidebar niewidoczny
- Na szerokości >=1024px sidebar widoczny
- MobileNav widoczny tylko na <1024px
  </verify>
  <done>Layout responsywny - sidebar ukryty na mobile, drawer dostępny</done>
</task>
...
1. Uruchom `npm run dev` i sprawdz w DevTools:
   - Mobile (375px): sidebar ukryty, hamburger widoczny, drawer otwiera się
   - Tablet (768px): sidebar ukryty, hamburger widoczny
   - Desktop (1024px+): sidebar widoczny, hamburger ukryty

2. Sprawdz nawigacje:
   - Wszystkie linki działają z drawera
   - Drawer zamyka się po kliknieciu linku
...
- [ ] Drawer otwiera się z lewej strony
- [ ] Wszystkie navItems dostępne w drawer
- [ ] Content pełna szerokość na mobile
