...
must_haves:
  truths:
    - "Wszystkie AI calls są trackowane przez Helicone"
    - "Koszty i latency widoczne w Helicone dashboard"
    - "Istniejaca funkcjonalność AI działa bez zmian"
  artifacts:
...
<objective>
Zintegrowac Helicone dla monitorowania kosztów i latency wszystkich AI calls.

Purpose: Spełnienie wymagania o dashboardzie kosztów AI - Helicone zapewnia automatyczny tracking
...
<task type="auto">
  ...
  <action>
1. Utworz folder src/lib/monitoring/ jeśli nie istnieje
...
<task type="auto">
  ...
  <action>
Zaktualizuj src/lib/ai/providers.ts aby uzyc Helicone gateway gdy klucz jest dostępny:
...
2. Warunkowe dodanie baseURL i headers gdy Helicone jest włączony
3. Registry używa provider instances zamiast default exports
4. Backward compatible - bez HELICONE_API_KEY działa jak wczesniej
  </action>
  <verify>
- `npm run build` przechodzi bez błędów
- Bez HELICONE_API_KEY - istniejącą funkcjonalność działa
- Z HELICONE_API_KEY - logi pokazują "Helicone monitoring enabled"
  </verify>
...
- [ ] Backward compatible - działa bez HELICONE_API_KEY
