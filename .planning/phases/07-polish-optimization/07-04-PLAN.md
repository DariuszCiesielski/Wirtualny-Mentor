...
must_haves:
  truths:
    ...
    - "Ciężkie komponenty są lazy-loaded (np. MentorChat)"
...
<objective>
Zoptymalizować performance przez bundle analysis i lazy loading ciężkich komponentów.

Purpose: Spełnienie kryterium "performance zoptymalizowany" - lazy loading, caching
...
- `ANALYZE=true npm run build` otwiera wizualizację
  </verify>
...
Kluczowe:
- `ssr: false` - MentorChat używa useChat hook (client-only)
- Loading skeleton dla lepszego UX podczas ładowania
- Zmniejsza initial bundle - MentorChat ładowany on-demand
  </action>
  <verify>
- Chat page używa dynamic import
- Loading skeleton wyświetla się przed zaladowaniem
- `npm run build` przechodzi
- Strona /courses/[id]/chat działa poprawnie
  </verify>
...
```
   - Powinien otworzyc się browser z wizualizacja bundle
   - Sprawdz rozmiary głównych chunks
...
2. Test lazy loading:
   ...
   - Sprawdz czy MentorChat laduje się osobnym chunk'iem
   - Loading skeleton powinien byc widoczny przez chwile

3. Sprawdz czy aplikacja działa:
   - Wszystkie strony się laduja
   - Chat działa normalnie po zaladowaniu
...
<verification>
1. `npm run build` przechodzi bez błędów i warnings

2. Bundle analysis:
   - `ANALYZE=true npm run build` otwiera wizualizację
   - Zidentyfikuj top 3 najwieksze dependencies
   - Zapisz baseline rozmiarów do porownan w przyszłości
...
3. Lazy loading verification:
   ...
   - Po zaladowaniu chat działa normalnie

4. Regression testing:
   - Wszystkie strony laduja się poprawnie
   - Quiz, notes, materials działają
   - Responsive design (z 07-01) działa
</verification>

<success_criteria>
- [ ] @next/bundle-analyzer zainstalowany i skonfigurowany
- [ ] ANALYZE=true npm run build działa
- [ ] MentorChat lazy-loaded z loading skeleton
- [ ] SWR zainstalowany (gotowy do uzycia w przyszłości)
- [ ] Build przechodzi bez błędów
